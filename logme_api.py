...
if not 'logid' in payload:
    ...
    logid = Popen(MYSQL + ['-e', sql], stdout=PIPE, stderr=DEVNULL, encoding='utf8').communicate()[0].rstrip('\n')
else:
	logid = payload['logid']

if 'exit_code' in payload:
	sql = "update log set log_exitcode="+ str(payload['exit_code']) +" where log_id="+ str(logid) +";\n"
	Popen(MYSQL + ['-e', sql], stdout=PIPE, stderr=DEVNULL, encoding='utf8').communicate()[0].rstrip('\n')
if payload['logtext']:
