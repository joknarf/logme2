...
class Reader{
  ...
  const static std::size_t width = 5000;
  ...
  void carriage_return(int offset){
    char_idx = offset;
  }
  ...
  bool id_and_process_control_char(unsigned char c, int offset){
    ...
    case 'J': case 'K': case 'L':carriage_return(0); line_feed(); return true;
    case 'M': carriage_return(offset); return true;
...
void Reader::read_from(std::istream& in){
  ...
  int char_offset=default_offset; // date time log prefix
  ...
  while(in){
    ...
    if(c+'@'=='J') { //FJO reset offset at newline ^J
        char_offset=default_offset;
    }
    if(state != SAW_OSC_EAT_2_BEL){ 
      if(id_and_process_control_char(c, char_offset)){
	continue;
      }
    }
    switch(state){
    ...
    case SAW_ESC: ///ESC ( ^] ) was seen (but no trailing ] or [ )
      ...
      case 'E': carriage_return(0); line_feed(); break; //Newline
	...
	set_state(SAW_NOTHING); 
	break;
//FJO      case 'm': ignore_CSI(); set_state(SAW_NOTHING); //Ignore char attributes
      case 'm': 
	int i;
	put_char(27); // ESC
	put_char(91); // [
	for(i=0;i<paridx;i++)
	  put_char(saveparams[i]);
	put_char(109); // m
	char_offset += 3 + paridx;
	set_state(SAW_NOTHING);
	break;
      case 'n': 
